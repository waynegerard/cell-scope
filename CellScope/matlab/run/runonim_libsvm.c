...
% runonim_libsvm.m: Script that runs algorithm on an image (or multiple images)
...
dohog = 0; % (1/0) = include/exclude HoG features
addpath('.\objlevel'); addpath('.\other')   % add paths to supporting files

% Load classifier model
if dohog
load('model_out_whog.mat')   % load trained object-classifier model with HoG features
else
load('model_out_wohog.mat') % model without HoG features
end
% no logistic regression model file needed because built into LibSVM

%% Let user choose images
mydir = './';
%mydir = 'C:\Users\jnchang\Documents\Fletcher Lab\CellScope Images\uganda_Jan12\posims\';
[fname_arr, pname] = uigetfile(fullfile(mydir,'*.tif'),'Select an image','MultiSelect','on');

% If multiple images are selected
if iscell(fname_arr)
len = size(fname_arr,2); % multiple files were selected, uigetfile gives cell array
else
len = 1;
end
...
% Set parameters
viewpats = 1;       % (1/0) view or don't view candidate patches, default: 1
sz = 24;            % size of patch that'll be extracted (must be even), default: 24
...
%     % For randomly chosen images
%         fname = fnames_keep{f};    % for randomly chosen images
